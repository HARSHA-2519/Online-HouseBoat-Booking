...
public partial class CUSTOMER_Payment_Recepit : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Application["BoatName"] != null)
        {
            lblBoatID.Text = Application["BoatName"].ToString();
        }
    ...
    {
        PayDet.BoatQuery = @"SELECT        ADV_PAY.Paydate,ADV_PAY.Bank_Name
FROM ADV_PAY INNER JOIN
                         BOAT_BOOKING ON ADV_PAY.Book_id = BOAT_BOOKING.Book_id where ADV_PAY.Book_id='" + Bookid+"'";
        PayDet.BoatReadData(PayDet.BoatQuery);
        if (PayDet.BoatDr.Read())
        {
            lblPayDate.Text= PayDet.BoatDr[0].ToString();
            lblBankName.Text = PayDet.BoatDr[1].ToString();
            PayDet.BoatDr.Close();
        }
        PayDet.BoatDr.Close();
    }
...
FROM BOAT_BOOKING INNER JOIN
    ...
    {
        PayDet.BoatQuery = @"SELECT        ADV_PAY.Adv_pid, ADV_PAY.CardType, ADV_PAY.Adv_amt, CUSTOMER_REG.Name, Boat_Booking.Book_id
FROM            ADV_PAY INNER JOIN
                         BOAT_BOOKING ON ADV_PAY.Book_id = BOAT_BOOKING.Book_id INNER JOIN
                         CUSTOMER_REG ON BOAT_BOOKING.Cust_id = CUSTOMER_REG.Cust_id where Boat_Booking.Book_id='"+Bookid+"'";
